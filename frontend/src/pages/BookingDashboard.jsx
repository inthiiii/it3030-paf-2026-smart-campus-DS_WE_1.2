          ...
          {myBookings.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {myBookings.map(booking => (
                <div key={booking.id} style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 2px 4px rgb(0 0 0 / 0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderLeft: `4px solid ${booking.status === 'CANCELLED' ? '#ef4444' : '#22c55e'}` }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.5rem' }}>
                    <span className={`status-badge status-${booking.status === 'CANCELLED' ? 'OUT_OF_SERVICE' : 'ACTIVE'}`}>
                      {booking.status}
                    </span>
